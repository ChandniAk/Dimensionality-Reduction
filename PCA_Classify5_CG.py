...
images = [   plt.imread(file)  for file in sorted(glob.glob("./Input/OPCarray_InputMask/*.png") )]
...
for idx in enumerate(images):
    #resizing images, mainly downsampling
...
images2 = [   plt.imread(file)  for file in sorted( glob.glob("./Output/etched/OPCArray/DRIE_shallow_SiSubs/*.png") )]
...
mlp = MLPClassifier(hidden_layer_sizes=(Nhid1,Nhid2 ), activation='relu', solver='adam', alpha=0.0001,
                    batch_size='auto', learning_rate='constant', learning_rate_init=1e-3, 
                    power_t=0.5, max_iter=20000, shuffle=True, random_state=1, tol=1e-4, 
...
print('execuation time is %f'  % (end - start))
...
for hh in range(0,45):
    plt.imsave('./output/etched/OPCArray/Temp1_real/train/'+ lll[hh]+'.png', y_train[hh],cmap=cm.gray)
    
    

for hh in range(0,5):
    plt.imsave('./output/etched/OPCArray/Temp1_real/test/'+ lll[hh+45]+'.png', y_test[hh],cmap=cm.gray)
...
for hh in range(0,45):
    plt.imsave('./output/etched/OPCArray/Temp2/train/'+ lll[hh]+'.png', ytrain_pred[hh],cmap=cm.gray)
    



for hh in range(0,5):
    plt.imsave('./output/etched/OPCArray/Temp2/test/'+ lll[hh+45]+'.png', ytest_pred[hh],cmap=cm.gray)
